...
class TencentAlchemyDataset(InMemoryDataset):
    ...
    def __init__(
        ...
        if self.mode == 'dev':
            self.stratify = torch.load(self.processed_paths[1])
        else:
            self.stratify = None
    ...
    def processed_file_names(self):
        return ['TencentAlchemy_%s.pt' % self.mode, 'num_heavy_atoms_%s.pt' % self.mode] if self.mode == 'dev' else ['TencentAlchemy_%s.pt' % self.mode, ]
