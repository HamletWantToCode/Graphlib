...
class TencentAlchemyDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        data_list = []
        class_vector = []
        for atom_dir in sdf_dir.glob("atom_*"):
            num_heavy_atoms = atom_dir.name.split('_')[1]
            for sdf_file in atom_dir.glob("**/*.sdf"):
                alchemy_data = self.sdf_graph_reader(sdf_file)
                if alchemy_data is not None:
                    data_list.append(alchemy_data)
                    class_vector.append(num_heavy_atoms)
        ...
        torch.save(class_vector, self.processed_dir+'/'+'class_vector.pt')
